 ...
 * 兼容4P
 */
public class ServerEventListener_CODE_4P_ROOM_JOIN implements ServerEventListener {
	...
	public void call(ClientSide clientSide, String data) {
		...
		if (room.getClientSideList().size() == 4) {
			String result = MapHelper.newInstance()
		...
		if (roomClientMap.size() == 4) {
			clientSide.setNext(roomClientList.getFirst());
			roomClientList.getFirst().setPre(clientSide);

			ServerEventListener.get(ServerEventCode.CODE_4P_GAME_STARTING).call(clientSide, String.valueOf(room.getId()));
			return;
		}
		...
		for (ClientSide client : roomClientMap.values()) {
			ChannelUtils.pushToClient(client.getChannel(), ClientEventCode.CODE_4P_ROOM_JOIN_SUCCESS, result);
		}
	...
	private void notifyWatcherJoinRoom(Room room, ClientSide clientSide) {
		for (ClientSide watcher : room.getWatcherList()) {
			ChannelUtils.pushToClient(watcher.getChannel(), ClientEventCode.CODE_4P_ROOM_JOIN_SUCCESS, clientSide.getNickname());
		}
	}
